...
Every other scenario here hands `writ` a space to walk. This one hands it a
...
[`writ-scheduling-verification`](https://github.com/writ-lang/writ-scheduling-verification); the schedules here are **frozen**, so the
verdicts below are exact and cannot change because somebody upgraded a solver.

## What `writ check` answers

```console
$ writ check timetable/timetable.writ --claims timetable/timetable.claims
states: 1   edges: 2
gaps: 2
...
in [`../libraries/school.lib.writ`](../libraries/school.lib.writ) is `fixed`,
...
one situation and `writ` spends its whole run evaluating the questions. Checking
is cheap in exactly the place generating is dear: asking `writ` to *produce* a
...
the same day — so the solver settled both by accident. `gap` is how a Writ model
...
```console
$ writ query timetable/timetable.writ clashes
clashes  (at state 0)
...
[`timetable-strict.writ`](timetable-strict.writ) is the same week solved again
...
```console
$ writ check timetable/timetable-strict.writ --claims timetable/timetable.claims
gaps: none
...
```console
$ writ compare timetable/timetable.writ timetable/timetable-strict.writ
...
"Five hours of maths a week" cannot be the numeral 5 — Writ has none. It is five
